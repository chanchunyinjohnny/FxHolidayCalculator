"""End-to-end test for `scripts/sources/cme_contracts.py`.
...
from scripts.sources.cme_contracts import fetch
...
def test_manual_row_survives_refresh(tmp_path):
    _copy_seed(REPO_ROOT / "data", tmp_path)
    # Pre-seed CME_contracts.json with a manual row at a non-standard month
    ...
    manual_blob = {
        ...
        "default_source": {
            ...
            "doc_title": "CME Group — FX Futures Contract Specifications",
        ...
        "contracts": [
            {
                "code": "ZAR_CUSTOM",
                "pair": "USD/ZAR",
                "product_name": "South African Rand Futures (manual)",
                ...
                "source": {
                    ...
                    "doc_title": "CME Notice 2026-08 South African Rand",
    ...
    (out_dir / "CME_contracts.json").write_text(json.dumps(manual_blob))

    fetch((2026, 2027), tmp_path)
    blob = json.loads((out_dir / "CME_contracts.json").read_text())
    codes_to_modes = {c["code"]: c["derivation_mode"] for c in blob["contracts"]}
    assert codes_to_modes.get("ZAR_CUSTOM") == "manual"
