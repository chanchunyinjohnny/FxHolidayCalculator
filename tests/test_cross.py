...
from fx_holiday_calculator.conventions.cross import relevant_venues, rtgs_calendar_set
...
def test_eurusd_with_ref_none_uses_two_calendars():
    cs = rtgs_calendar_set(
        parse_pair("EUR/USD"),
        ref="none",
        calendars={"EUR": _empty("EUR"), "USD": _empty("USD"), "JPY": _empty("JPY")},
    )
...
def test_eurjpy_with_ref_usd_unions_three():
    cs = rtgs_calendar_set(
        parse_pair("EUR/JPY"),
        ref="USD",
        calendars={"EUR": _empty("EUR"), "JPY": _empty("JPY"), "USD": _empty("USD")},
    )
...
def test_usdjpy_with_ref_usd_dedups():
    cs = rtgs_calendar_set(
        parse_pair("USD/JPY"), ref="USD", calendars={"USD": _empty("USD"), "JPY": _empty("JPY")}
    )
