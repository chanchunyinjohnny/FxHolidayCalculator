"""End-to-end test for `scripts/sources/sgx_contracts.py`.
...
from scripts.sources.sgx_contracts import fetch
...
def test_manual_row_survives_refresh(tmp_path):
    ...
    manual_blob = {
        ...
        "default_source": {
            "url": "https://www.sgx.com/derivatives/products/currency-futures",
            "doc_title": "SGX — Currency Futures Contract Specifications",
            "fetched_at": "2026-01-01T00:00:00Z",
            "fetcher": "scripts/sources/sgx_contracts.py@v1",
        ...
        "contracts": [
            {
                "code": "THB_CUSTOM",
                "pair": "USD/THB",
                "product_name": "Thai Baht Futures (manual)",
                ...
                "source": {
                    "url": "https://www.sgx.com/derivatives/...",
                    "doc_title": "SGX Notice 2026-08 Thai Baht",
    ...
    (out_dir / "SGX_contracts.json").write_text(json.dumps(manual_blob))

    fetch((2026, 2027), tmp_path)
    blob = json.loads((out_dir / "SGX_contracts.json").read_text())
    codes_to_modes = {c["code"]: c["derivation_mode"] for c in blob["contracts"]}
    assert codes_to_modes.get("THB_CUSTOM") == "manual"
